...
router.get("/analysis", async (req, res) => {
  try {
    ...
    const insights =
      flowResponse.outputs[0].outputs[0].results.message.text;

    const insights2 = JSON.parse(insights.trim().substring(7, insights.length - 4));

    // console.log("Langflow response: ",insights2);

    // res.render("analysis", { title: "Post Analysis", data: results, insights });
